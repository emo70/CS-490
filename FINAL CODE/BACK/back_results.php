...
if ($data["requestType"]=="autograder") {
	$sql = "INSERT INTO results".
		"(username,exam_id,takenOn,released) ".
		"values(?, ?, ?, ?)";
	$stmt = mysqli_stmt_init($conn);
	$sql2 = "INSERT INTO answers".
		"(question_id, result_id, answer, maxPoints, autograderComment) ".
		"values(?, ?, ?, ?, ?)";
...
elseif ($data["requestType"]=="pendingResults" || 
	...
	$sql = "SELECT r.id, r.username, e.name examName, r.takenOn, r.released ".
...
elseif ($data["requestType"]=="releaseResult") {
	$sql = "UPDATE results SET released = 1 WHERE id=?";
	$stmt = mysqli_stmt_init($conn);
